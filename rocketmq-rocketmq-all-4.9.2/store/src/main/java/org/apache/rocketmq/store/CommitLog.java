...
public class CommitLog {
    ...
    public void recoverNormally(long maxPhyOffsetOfConsumeQueue) {
        ...
        if (!mappedFiles.isEmpty()) {
            // Began to recover from the last third file    Broker正常停止再重启时,从倒数第三个开始恢复,如果不足3个文件,则从第一个文件开始恢复。
            int index = mappedFiles.size() - 3;
            if (index < 0)
                index = 0;

            // 取出mappedFile、byteBuffer、上次处理偏移量processOffset
     ...
     * 异常文件恢复步骤与正常停止文件恢复流程基本相同，验证逻辑相似，其主要差别有两个：
     * 首先，正常停止默认从倒数第三个文件开始进行恢复，而异常停止则需要从最后一个文件往前走，找到第一个消息存储正常的文件;
     * 其次，如果CommitLog目录没有消息文件，且消息消费队列目录下存在文件，则需要销毁。
    ...
    public void recoverAbnormally(long maxPhyOffsetOfConsumeQueue) {
        ...
        final List<MappedFile> mappedFiles = this.mappedFileQueue.getMappedFiles();
        if (!mappedFiles.isEmpty()) {
            // Looking beginning to recover from which file  异常停止从最后一个文件往前走
        ...
        // Commitlog case files are deleted     未找到mappedFile,重置flushWhere、committedWhere都为0，销毁消息队列文件
        else {
            log.warn("The commitlog files are deleted, and delete the consume queue files");
            this.mappedFileQueue.setFlushedWhere(0);
            this.mappedFileQueue.setCommittedWhere(0);
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessage(final MessageExtBrokerInner msg) {
        ...
        if (tranType == MessageSysFlag.TRANSACTION_NOT_TYPE
            ...
            // Delay Delivery   处理延迟消息的主题和队列，延迟消息不需要立即存储需要另外其它操作
        ...
        putMessageLock.lock(); //spin or ReentrantLock ,depending on store config   spin或ReentrantLock，取决于存储配置
        try {
            // 获得最后一个mappedFile，rocketMQ是文件内存映射机制存储，commitLog映射MappedFile进行存储
            MappedFile mappedFile = this.mappedFileQueue.getLastMappedFile();
            long beginLockTimestamp = this.defaultMessageStore.getSystemClock().now();
            this.beginTimeInLock = beginLockTimestamp;

            // Here settings are stored timestamp, in order to ensure an orderly    这里的设置都存储了时间戳，以确保有序
            // global
            msg.setStoreTimestamp(beginLockTimestamp);

            // mappedFile如果为null或已写满,则创建新的mappedFile文件
            ...
            // 将消息写入内存中，将消息追加到内存映射文件mappedFile中
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessages(final MessageExtBatch messageExtBatch) {
        ...
        // 处理延迟消息的主题和队列，延迟消息不需要立即存储需要另外其它操作
        ...
        //fine-grained lock instead of the coarse-grained  细粒度锁而不是粗粒度锁
        ...
        // 加锁，对mappedFile进行映射存储,spin或ReentrantLock，取决于存储配置
        putMessageLock.lock();
        try {
            long beginLockTimestamp = this.defaultMessageStore.getSystemClock().now();
            this.beginTimeInLock = beginLockTimestamp;
    ...
    public CompletableFuture<PutMessageStatus> submitFlushRequest(AppendMessageResult result, MessageExt messageExt) {
        // Synchronization flush  判断是同步刷盘
        if (FlushDiskType.SYNC_FLUSH == this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
            //封装刷盘请求对象
            final GroupCommitService service = (GroupCommitService) this.flushCommitLogService;
            if (messageExt.isWaitStoreMsgOK()) {
                // 设置写偏移量和超时时间，线程阻塞5秒，等待刷盘结束
        ...
        // Asynchronous flush   判断是异步刷盘
        else {
            // 判断是否开启transientStorePoolEnable，没打开则直接刷盘，否则要先提交到对外内存ByteBuffer
    ...
    class CommitRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                ...
                //与上次提交间隔超过commitDataThoroughInterval,则忽略提交commitDataThoroughInterval参数,直接提交
                ...
                try {
                    ...
                    if (!result) {
                        ...
                        //now wake up flush thread.  唤醒刷盘线程
                        flushCommitLogService.wakeup();
                    }
    ...
    public static class GroupCommitRequest {
        ...
        private final long startTimestamp = System.currentTimeMillis();// 刷盘开始时间
        private long timeoutMillis = Long.MAX_VALUE;// 刷盘超时时间
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            // PHY OFFSET 获得当前文件写入位置
            long wroteOffset = fileFromOffset + byteBuffer.position();

            // 创建提供messageId函数，生成消息Id
            ...
            // Determines whether there is sufficient free space 确定是否有足够的空闲空间,消息是如果没有足够的存储空间则新创建CommitLog文件
            ...
            final long beginTimeMills = CommitLog.this.defaultMessageStore.now();
            // Write messages to the queue buffer 此处是将消息写入到内存，并不是持久化到磁盘中
            ...
            switch (tranType) {
                ...
                case MessageSysFlag.TRANSACTION_NOT_TYPE:
                case MessageSysFlag.TRANSACTION_COMMIT_TYPE:
                    // The next update ConsumeQueue information  更新消息队列偏移量
                    CommitLog.this.topicQueueTable.put(key, ++queueOffset);
                    break;
                default:
                    break;
